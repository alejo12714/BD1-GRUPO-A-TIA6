...
CREATE TABLE evaluacion (
    ...
    observacion_evaluacion VARCHAR(500) NOT NULL,
    cod_evaluador INT NOT NULL,
    cod_proyecto INT, -- Puede ser NULL al inicio
    hora TIME NOT NULL,
    fecha DATE NOT NULL,
    c1 DECIMAL (2,1) NOT NULL CHECK (c1 >= 0 AND c1 <= 5) DEFAULT 0,
    c2 DECIMAL (2,1) NOT NULL CHECK (c2 >= 0 AND c2 <= 5) DEFAULT 0,
    c3 DECIMAL (2,1) NOT NULL CHECK (c3 >= 0 AND c3 <= 5) DEFAULT 0,
    c4 DECIMAL (2,1) NOT NULL CHECK (c4 >= 0 AND c4 <= 5) DEFAULT 0,
    c5 DECIMAL (2,1) NOT NULL CHECK (c5 >= 0 AND c5 <= 5) DEFAULT 0,
    c6 DECIMAL (2,1) NOT NULL CHECK (c6 >= 0 AND c6 <= 5) DEFAULT 0,
    c7 DECIMAL (2,1) NOT NULL CHECK (c7 >= 0 AND c7 <= 5) DEFAULT 0,
    c8 DECIMAL (2,1) NOT NULL CHECK (c8 >= 0 AND c8 <= 5) DEFAULT 0,
    c9 DECIMAL (2,1) NOT NULL CHECK (c9 >= 0 AND c9 <= 5) DEFAULT 0,
    c10 DECIMAL (2,1) NOT NULL CHECK (c10 >= 0 AND c10 <= 5) DEFAULT 0,
    -- ¡Aquí está la corrección! Agregué 'GENERATED ALWAYS'
    ponderacion_evaluacion DECIMAL(4,2) GENERATED ALWAYS AS (
        (c1 + c2 + c3 + c4 + c5 + c6 + c7 + c8 + c9 + c10) / 10.0
    ) STORED,
