...
FROM
    proyecto AS proy
JOIN
    tipo_proyecto AS tipo ON proy.cod_tipo_proyecto = tipo.cod_tipo_proyecto
JOIN
    estudiante AS estu ON proy.cod_proyecto = estu.cod_Proyecto -- Assuming a project must have at least one student to link to a program
JOIN
...
JOIN
    tipo_proyecto AS tipo ON proy.cod_tipo_proyecto = tipo.cod_tipo_proyecto
INNER JOIN -- ¡Cambio aquí! Solo proyectos con estudiantes
    estudiante AS estu ON proy.cod_proyecto = estu.cod_Proyecto
LEFT JOIN -- Estos pueden seguir siendo LEFT JOIN si la data es consistente
    programa_academico AS proa ON estu.cod_programa_academico = proa.cod_programa
LEFT JOIN
    departamento AS depa ON proa.cod_departamento = depa.cod_departamento
LEFT JOIN
    facultad AS fac ON depa.cod_facultad = fac.cod_facultad
INNER JOIN -- ¡Cambio aquí! Solo estudiantes que estén en un curso
    "curso-estudiante" AS curest ON estu.cod_estudiante = curest.cod_estudiante
INNER JOIN -- ¡Cambio aquí! Solo si el curso existe
    curso AS curs ON curest.cod_curso = curs.cod_curso
INNER JOIN -- ¡Cambio aquí! Solo si la asignatura existe
    asignatura AS asig ON curs.cod_asignatura = asig.cod_asignatura
INNER JOIN -- ¡Cambio aquí! Solo si el docente existe
    docente AS doce ON curs.cod_docente = doce.cod_docente
ORDER BY
    proy.cod_proyecto;
...
FROM
    proyecto AS proy
JOIN
    evaluacion AS eval ON proy.cod_evaluacion = eval.cod_evaluacion
JOIN
    evaluadores AS evalu ON eval.cod_evaluador = evalu.cod_evaluador
JOIN
    institucion AS inst ON evalu.cod_institucion = inst.cod_institucion
ORDER BY
    proy.cod_proyecto, eval.cod_evaluacion;
...
FROM
    facultad AS fac
JOIN
    departamento AS depa ON fac.cod_facultad = depa.cod_facultad
JOIN
    programa_academico AS proa ON depa.cod_departamento = proa.cod_departamento
LEFT JOIN
    estudiante AS estu ON proa.cod_programa = estu.cod_programa_academico
LEFT JOIN
    proyecto AS proy ON estu.cod_Proyecto = proy.cod_proyecto
GROUP BY
    fac.cod_facultad,
    fac.nombre_facultad,
    proa.cod_programa,
...
FROM
    tipo_proyecto AS tipo
LEFT JOIN
    proyecto AS proy ON tipo.cod_tipo_proyecto = proy.cod_tipo_proyecto
LEFT JOIN
    "proyecto-curso" AS procu ON proy.cod_proyecto = procu.cod_proyecto
LEFT JOIN
    curso AS curs ON procu.cod_curso = curs.cod_curso
LEFT JOIN
    asignatura AS asig ON curs.cod_asignatura = asig.cod_asignatura
LEFT JOIN
    programa_academico AS proa ON asig.cod_programa = proa.cod_programa
LEFT JOIN
    departamento AS depa ON proa.cod_departamento = depa.cod_departamento
LEFT JOIN
    facultad AS fac ON depa.cod_facultad = fac.cod_facultad
GROUP BY -- Grouping to avoid duplicate rows for the same assignment/group linked to multiple projects of the same type
    tipo.tipo_proyecto,
    fac.nombre_facultad,
    proa.nombre_programa_academico,
    asig.nombre_asignatura,
...
FROM
    evaluadores AS evalu
JOIN
    institucion AS insti ON evalu.cod_institucion = insti.cod_institucion
LEFT JOIN
    evaluacion AS eval ON evalu.cod_evaluador = eval.cod_evaluador
LEFT JOIN
    proyecto AS proy ON eval.cod_proyecto = proy.cod_proyecto
LEFT JOIN
    tipo_proyecto AS tipo ON proy.cod_tipo_proyecto = tipo.cod_tipo_proyecto
LEFT JOIN
    estudiante AS estu ON proy.cod_proyecto = estu.cod_Proyecto -- Link project to student to find program/faculty
LEFT JOIN
    programa_academico AS proa ON estu.cod_programa_academico = proa.cod_programa
LEFT JOIN
    departamento AS depa ON proa.cod_departamento = depa.cod_departamento
...
FROM
    usuario AS usua
JOIN
    "usuario-rol" AS usur ON usua.cod_usuario = usur.cod_usuario
JOIN
    rol AS role ON usur.cod_rol = role.cod_rol
ORDER BY
    usua.usuario; -- Ordenar por el nombre de usuario
...
GROUP BY
    ...
    fac.nombre_facultad
HAVING
    COUNT(DISTINCT proy.cod_proyecto) > 0 -- Only include faculties with at least one project
ORDER BY
    cantidad_proyectos DESC, fac.nombre_facultad;